...
export function resolveVoiceCallConfig(config: VoiceCallConfig): VoiceCallConfig {
  ...
  if (resolved.provider === "livekit") {
    ...
    resolved.livekit.apiKey = resolved.livekit.apiKey ?? process.env.LIVEKIT_API_KEY;
    resolved.livekit.apiSecret = resolved.livekit.apiSecret ?? process.env.LIVEKIT_API_SECRET;
    resolved.livekit.wsUrl = resolved.livekit.wsUrl ?? process.env.LIVEKIT_URL;
  }
  ...
  // Note: older configs may still include allowNgrokFreeTier; treat it as an alias for the bypass.
  resolved.tunnel.allowNgrokFreeTierLoopbackBypass = Boolean(
    resolved.tunnel.allowNgrokFreeTierLoopbackBypass ??
    (resolved.tunnel as { allowNgrokFreeTier?: boolean }).allowNgrokFreeTier ??
    false,
  );
